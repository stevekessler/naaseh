...
Run `npm run validate` for type checking, linting, unit tests, and builds, and `npm run test:python` for the operator-command tests. Before AWS work, use `npm run validate:pre-aws`; add Chromium/WebKit with `npm run validate:pre-aws:browsers`. On a Mac with Safari Technology Preview installed, run `npm run test:safari-preview` for the independent native-WebDriver smoke check. `VERBOSE_LOGGING` is enabled only by the literal value `true`; it defaults off. Logs must never include passwords, PINs, cookies, tokens, cryptographic material, or protected task content.
