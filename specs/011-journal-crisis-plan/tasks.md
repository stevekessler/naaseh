...
- [X] T099 Run the complete repository validation and confirm the hosted PR check stays at or below ten minutes with reasonable headroom under the workflow timeout in `.github/workflows/validate.yml`; record the run link and duration in `specs/011-journal-crisis-plan/quickstart.md`
...
- [X] T107 Run focused broker/infrastructure/smoke tests and the complete `npm run validate:pre-aws:browsers` gate; then commit only intended files, open the PR, confirm the hosted required check remains below ten minutes, and record evidence
